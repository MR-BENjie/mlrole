...
class ParallelRunner:
    ...
    def reset(self):
        ...
        for parent_conn in self.parent_conns:
            ...
            pre_transition_data["enemy_obs"].append(data['enemy_obs'])
        ...
        while True:
            ...
            for idx, parent_conn in enumerate(self.parent_conns):
                if not terminated[idx]:
                    ...
                    post_transition_data["pre_types"].append(data['pre_types'])
                    post_transition_data["target_types"].append(data["target_types"])
                    post_transition_data["accurancy"].append((data["accurancy"],))

                    pre_types_list.append(data['pre_types'])
                    target_types_list.append(data["target_types"].to(self.args.device))

                    accuracy_list.append(data["accurancy"])
...
def env_worker(remote, env_fn):
    ...
    while True:
        ...
        if cmd == "step":
            actions = data
            pre_types, target_types = env.get_type_judge_result()
            accurancy = env.get_type_judege_accurancy()
        ...
        elif cmd == "update_judge_model":
            remote.send(env.update_judge_model(data))
        else:

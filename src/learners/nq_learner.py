...
class NQLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        #(add) if train the judge module
        self.judge_model_used = args.judge_model_used
        self.judge_model_encode = role_Encode(in_features=args.obs_shape, mlp_hiddens=[128,512], mlp_out_features=256, hiddens=128, )
        ...
        if self.judge_model_used:
            ...
            if self.args.optimizer == 'adam':
                self.judge_optimiser = Adam(params=(list(self.judge_model_encode.parameters())+list(self.judge_model_node.parameters())), lr=args.lr, weight_decay=getattr(args, "weight_decay", 0))
            else:
                self.judge_optimiser = RMSprop(params=(list(self.judge_model_node.parameters())+list(self.judge_model_node.parameters())), lr=args.lr, alpha=args.optim_alpha, eps=args.optim_eps)
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, per_weight=None):
        ...
        if self.judge_model_used:
            pre_types = batch["pre_types"]
            target_types = batch["target_types"]
            accurancy = batch["accurancy"]
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            if self.judge_model_used:
                self.logger.log_stat("judge_model_accuracy", accurancy.mean().item(), t_env)
    ...
    def cuda(self):
        ...
        self.judge_model_encode.cuda()
        self.judge_model_node.cuda()
    def save_models(self, path):
        ...
        if self.judge_model_used:
            th.save(self.judge_model_encode.state_dict(), "{}/judge_model_encode.th".format(path))
            th.save(self.judge_model_node.state_dict(), "{}/judge_model_node.th".format(path))
            th.save(self.judge_optimiser.state_dict(), "{}/judge_opt.th".format(path))
    ...
    def load_models(self, path):
        ...
        if self.judge_model_used:
            self.judge_model_encode.load_state_dict(th.load("{}/judge_model_encode.th".format(path)))
            self.judge_model_node.load_state_dict(th.load("{}/judge_model_node.th".format(path)))
            self.judge_optimiser.load_state_dict(th.load("{}/judge_opt.th".format(path)))
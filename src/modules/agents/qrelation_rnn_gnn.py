...
# dis_idx: the index of the distance in the observation
def adjacency_and_create_graph(ob, n_agents, dis_idx, graph_library):
    ...
    dis_l = ob[:, dis_idx]
    agent_ids_new = torch.where(dis_l > 0, agent_ids, torch.zeros_like(agent_ids)-1)
    print(agent_ids_new)
    agent_ids_new.to(ob.device)

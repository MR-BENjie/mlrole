...
class StarCraft2Env(MultiAgentEnv):
    ...
    def __init__(
        ...
        type_kinds = 4,
        ally_hind_ids = [1,3,4],
        enemy_hind_ids = [1,3,4],
        ally_public_ids = [0,2],
        enemy_public_ids = [0,2],
        ally_id_type_map = [2, 2, 3, 3,3],
        enemy_id_type_map = [0, 0, 1, 1, 1],
        sc2_mlrole = False,
        ...
        self.device = device
        self.random_obs_seed = 0
        self.type_kinds = type_kinds
        self.ally_hind_ids = ally_hind_ids
        self.enemy_hind_ids = enemy_hind_ids
        self.ally_public_ids = ally_public_ids
        self.enemy_public_ids = enemy_public_ids
        self.ally_id_type_map = ally_id_type_map
        self.enemy_id_type_map = enemy_id_type_map
        self.sc2_mlrole = sc2_mlrole
        self.our_method_used = our_method_used # if use our method to jude the role of the hind agent, and refact the obs using the judement
        self.jude_role_mode = jude_role_mode # jude_role_mode : a dict for {agent_id (hidden role): pre_role}
        self.judge_model_Encode = None
        self.judge_model_MlroleNode = None
        self.use_judge_model = use_judge_model
        self.pre_types = None
        ...
        if self.sc2_mlrole:
            ...
            print("ally_public_ids: "+str(ally_public_ids))
            print("ally_id_type_map: "+str(ally_id_type_map))

            self.hind_type = list()
            for a_id in self.ally_hind_ids:
                self.hind_type.append(self.ally_id_type_map[a_id])
            for e_id in self.enemy_hind_ids:
                self.hind_type.append(self.enemy_id_type_map[e_id])
    ...
    def get_obs_agent(self, agent_id):
        ...
        if self.sc2_mlrole:
            if unit.health > 0:  # otherwise dead, return all zeros
                if self.use_judge_model:
                    self.judge_role(agent_id)
                ...
                for i, al_id in enumerate(al_ids):
                    ...
                    if al_id in self.ally_hind_ids:
                        ally_hid_np.append(i)
                    else:
                        ally_pid_np.append(i)
                    ally_npid_map[al_id] = i
                ...
                if not self.use_judge_model:
                    if self.random_obs_seed==0:
                        agent_obs = np.concatenate(
                            (
                                move_feats.flatten(),
                                enemy_feats[self.enemy_public_ids,:].flatten(),
                                ally_feats[ally_pid_np, :].flatten(),
                                enemy_feats[self.enemy_hind_ids, :].flatten(),
                                ally_feats[ally_hid_np,:].flatten(),
                                own_feats.flatten(),
                            )
                        )
                        self.random_obs_seed = 1
                    elif self.random_obs_seed==1:
                        agent_obs = np.concatenate(
                            (
                                move_feats.flatten(),
                                enemy_feats[self.enemy_public_ids, :].flatten(),
                        ...
                        agent_obs = np.concatenate(
                            (
                                ...
                                enemy_feats[self.enemy_public_ids, :].flatten(),
                                ally_feats[ally_pid_np, :].flatten(),
                                np.concatenate([np.concatenate((enemy_feats[en,:],ally_feats[al,:])) for en,al in zip(self.enemy_hind_ids[:len(ally_hid_np)],ally_hid_np)]),
                                enemy_feats[self.enemy_hind_ids[len(ally_hid_np):],:].flatten(),
                                own_feats.flatten(),
                            )
                        )
                        self.random_obs_seed = 0
                else:
                    agent_id_feats = np.zeros((ally_feats_dim[1]), dtype=np.float32)
                    if self.use_judge_model:
                        al_unit = unit
                        al_x = al_unit.pos.x
                        al_y = al_unit.pos.y
                        ...
                        agent_id_feats[ 0] = 1  # visible
                        agent_id_feats[ 1] = dist / sight_range  # distance
                        agent_id_feats[ 2] = (al_x - x) / sight_range  # relative X
                        agent_id_feats[ 3] = (al_y - y) / sight_range  # relative Y
                        ...
                        if self.unit_type_bits > 0:
                            type_id = self.get_unit_type_id(al_unit, True)
                            agent_id_feats[ind + type_id] = 1
                            ind += self.unit_type_bits

                        if self.obs_last_action:
                            agent_id_feats[ind:] = self.last_action[agent_id]
                    # jude_role_mode : a dict for {agent_id (hidden role): pre_role}
                    ally_stalker_feats = []
                    ally_zealot_feats = []
                    enemy_stalker_feats = []
                    enemy_zealot_feats = []
                    for enetity in self.jude_role_mode.items():
                        input_agent_feats = np.zeros((ally_feats_dim[1]), dtype=np.float32)
                        if enetity[0] < self.n_agents:
                            if enetity[0] == agent_id:
                                continue
                            else:
                                input_agent_feats = ally_feats[ally_npid_map[enetity[0]],:]
                        else:
                            enetity_id = enetity[0]-self.n_agents
                            input_agent_feats = enemy_feats[enetity_id, :]

                        if enetity[1] == 0 :
                            # jude enemy stalker
                            enemy_stalker_feats.append(input_agent_feats)
                        elif enetity[1] == 1:
                            #jude enemy zealot
                            enemy_zealot_feats.append(input_agent_feats)
                        elif enetity[1] ==2 :
                            #jude ally stalker
                            ally_stalker_feats.append(input_agent_feats)
                        else:
                            # jude enemy zealot
                            ally_zealot_feats.append(input_agent_feats)
                    agent_obs_list = list()
                    agent_obs_list.append(move_feats.flatten())
                    if len(enemy_stalker_feats)!=0 :
                        agent_obs_list.append(np.concatenate(enemy_stalker_feats).flatten())
                    agent_obs_list.append(enemy_feats[self.enemy_public_ids, :].flatten())
                    if len(enemy_zealot_feats)!=0 :
                        agent_obs_list.append(np.concatenate(enemy_zealot_feats).flatten())
                    if len(ally_stalker_feats)!=0:
                        agent_obs_list.append(np.concatenate(ally_stalker_feats).flatten())
                    agent_obs_list.append(ally_feats[ally_pid_np, :].flatten())
                    if len(ally_zealot_feats)!=0:
                        agent_obs_list.append(np.concatenate(ally_zealot_feats).flatten())
                    agent_obs_list.append(own_feats.flatten())
    ...
    def update_judge_model(self, model):
        self.judge_model_Encode = model['Encode']
        self.judge_model_MlroleNode = model['MlroleNode']

    def judge_role(self,agent_id):
        #jude_role_mode: a dict for {agent_id(hidden role): pre_role}
        self.jude_role_mode = None
        self.use_judge_model = False
        jude_role_mode_ = dict()
        ally_obs = self.get_obs()
        enemy_obs = self.get_enemy_obs()
        hind_obs = []

        for id_obs , obs in enumerate(ally_obs):
            if id_obs != agent_id:
                with torch.no_grad():
                    hind_obs.append(self.judge_model_Encode(torch.unsqueeze(torch.tensor(obs,dtype=torch.float32, device=self.device),dim=0)))
            else:
                hind_obs.append(self.judge_model_Encode(
                    torch.unsqueeze(torch.tensor(obs, dtype=torch.float32, device=self.device), dim=0)))
        ...
        input_node = dict()
        input_node['hidden'] = hind_obs[agent_id]
        input_node['types'] = dict()
        for type in range(self.type_kinds):
            input_node['types'][type] = list()
        for al_id in self.ally_public_ids:
            input_node['types'][self.ally_id_type_map[al_id]].append(hind_obs[al_id])
        for en_id in self.enemy_public_ids:
            input_node['types'][self.enemy_id_type_map[en_id]].append(hind_obs[en_id+self.n_agents])
        ...
        pre_types = types
        target_types = torch.vstack(target_types)
        ...
        accurancy = 0.0
        hind_ally_obs_number = len(self.ally_hind_ids)
        types = types.clone().detach()
        for type in self.hind_type:
            id_a = int(torch.argmax(types, dim=0)[type])
            types[id_a] = -1
            if id_a < hind_ally_obs_number:
                if type == self.ally_id_type_map[self.ally_hind_ids[id_a]]:
                    accurancy += 1.0
                jude_role_mode_[self.ally_hind_ids[id_a]] = type
            else:
                if type == self.enemy_id_type_map[self.enemy_hind_ids[id_a-hind_ally_obs_number]]:
                    accurancy += 1.0
                jude_role_mode_[self.enemy_hind_ids[id_a-hind_ally_obs_number]+self.n_agents] = type
        self.jude_role_mode = jude_role_mode_
        self.use_judge_model = True
        self.accurancy = accurancy/(hind_ally_obs_number+len(self.enemy_hind_ids))
    ...
    def get_type_judege_accurancy(self):
        return self.accurancy
    def get_obs_enemy_agent(self, agent_id):
        """Returns observation for agent_id. The observation is composed of:
        ...
        if unit.health > 0:  # otherwise dead, return all zeros
            ...
            for e_id in range(self.n_agents):
                e_unit = self.get_unit_by_id(e_id)
    ...
    def get_enemy_obs(self):
        enemy_obs = [self.get_obs_enemy_agent(i) for i in range(self.n_agents)]
        return enemy_obs

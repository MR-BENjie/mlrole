...
def _get_config_byname(config_name, subfolder):
    if config_name is not None:
        with open(os.path.join(os.path.dirname(__file__), "config", subfolder, "{}.yaml".format(config_name)), "r") as f:
            try:
                config_dict = yaml.load(f)
...
if __name__ == '__main__':
    ...
    with open(os.path.join(os.path.dirname(__file__), "config", "battle_default.yaml"), "r") as f:
        try:
            config_dict = yaml.load(f)
        except yaml.YAMLError as exc:
    ...
    if config_dict["model_type"] == "marl":
        alg_config_1  = _get_config_byname(config_dict['config'], "algs")
        config_dict = recursive_dict_update(config_dict, alg_config_1)
    if config_dict["model_type_2"] == "marl":
        alg_config_1 = _get_config_byname(config_dict['config_2'], "algs")
        config_dict = recursive_dict_update(config_dict, alg_config_1)
